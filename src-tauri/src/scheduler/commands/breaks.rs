...
pub struct QuickStatus {
    pub state: QuickPanelState,
    pub remaining_secs: Option<u64>,
    pub pause_remaining_secs: Option<u64>,
}

pub fn decide_quick_panel_snapshot(
    paused: bool,
    on_break: bool,
    bedtime_active: bool,
    suppressed: bool,
    micro_or_long_enabled: bool,
    countdown_secs: Option<u64>,
) -> (QuickPanelState, Option<u64>) {
    if paused {
        (QuickPanelState::Paused, None)
    } else if on_break {
        (QuickPanelState::OnBreak, None)
    } else if bedtime_active {
        (QuickPanelState::Bedtime, None)
    } else if suppressed {
        (QuickPanelState::Suppressed, None)
    } else if !micro_or_long_enabled {
        (QuickPanelState::Disabled, None)
    } else if let Some(secs) = countdown_secs {
        (QuickPanelState::Running, Some(secs))
    } else {
        (QuickPanelState::Idle, None)
    }
}

pub async fn get_quick_status_impl(scheduler: &Scheduler) -> QuickStatus {
    let s = scheduler.settings.lock().await.clone();
    let (paused, pause_remaining) = {
        let state = scheduler.pause_state.lock().await;
        match &*state {
            PauseState::Running => (false, None),
            PauseState::PausedUntil(None) => (true, None),
            PauseState::PausedUntil(Some(t)) => {
                let now = Instant::now();
                let remaining = if *t > now { (*t - now).as_secs() } else { 0 };
                (true, Some(remaining))
    ...
    let bedtime_active = s.bedtime_enabled
        && crate::scheduler::timers::in_window(
            crate::scheduler::timers::current_minutes(),
            s.bedtime_start_minutes,
            s.bedtime_end_minutes,
        );
    let suppress_reason = crate::scheduler::SuppressReason::from_u8(
        scheduler
            .auto_suppress_reason
            .load(std::sync::atomic::Ordering::Relaxed),
    );
    let on_break = crate::scheduler::lock_current_break(&scheduler.current_break).is_some();
    let micro_or_long_enabled = s.micro_enabled || s.long_enabled;
    ...
    let countdown_secs = if paused || bedtime_active || on_break || suppress_reason.is_some() {
        ...
        super::super::tray_countdown::next_break_countdown_secs(
            &s,
            &t,
            Instant::now(),
    ...
    let (state, remaining) = decide_quick_panel_snapshot(
        paused,
        on_break,
        bedtime_active,
        suppress_reason.is_some(),
        micro_or_long_enabled,
        countdown_secs,
    );
...
#[tauri::command]
pub async fn get_quick_status(
    scheduler: tauri::State<'_, Scheduler>,
) -> Result<QuickStatus, String> {
    Ok(get_quick_status_impl(scheduler.inner()).await)
}
...
mod tests {
    ...
    #[test]
    fn decide_quick_panel_snapshot_states() {
        use super::{decide_quick_panel_snapshot, QuickPanelState};

        assert_eq!(
            decide_quick_panel_snapshot(true, true, true, true, true, Some(100)),
            (QuickPanelState::Paused, None)
        );
        assert_eq!(
            decide_quick_panel_snapshot(false, true, true, true, true, Some(100)),
            (QuickPanelState::OnBreak, None)
        );
        assert_eq!(
            decide_quick_panel_snapshot(false, false, true, true, true, Some(100)),
            (QuickPanelState::Bedtime, None)
        );
        assert_eq!(
            decide_quick_panel_snapshot(false, false, false, true, true, Some(100)),
            (QuickPanelState::Suppressed, None)
        );
        assert_eq!(
            decide_quick_panel_snapshot(false, false, false, false, false, None),
            (QuickPanelState::Disabled, None)
        );
        assert_eq!(
            decide_quick_panel_snapshot(false, false, false, false, true, Some(277)),
            (QuickPanelState::Running, Some(277))
        );
        assert_eq!(
            decide_quick_panel_snapshot(false, false, false, false, true, None),
            (QuickPanelState::Idle, None)
        );
    }

    #[tokio::test]
    async fn get_quick_status_impl_running_even_when_tray_text_is_off() {
        let s = Settings {
        ...
        let (_dir, sched) = test_scheduler(s);
        let status = get_quick_status_impl(&sched).await;
        assert_eq!(status.state, QuickPanelState::Running);
        let secs = status.remaining_secs.expect("running countdown");
        assert!(secs <= micro, "{secs} <= {micro}");
        assert!(micro - secs < 5, "fresh anchor → close to full interval");
        assert!(status.pause_remaining_secs.is_none());
    }

    #[tokio::test]
    async fn get_quick_status_impl_paused_reports_remaining() {
        let (_dir, sched) = test_scheduler(Settings::default());
        *sched.pause_state.lock().await =
            PauseState::PausedUntil(Some(Instant::now() + Duration::from_secs(90)));
        let status = get_quick_status_impl(&sched).await;
        assert_eq!(status.state, QuickPanelState::Paused);
        let rem = status.pause_remaining_secs.expect("timed pause");
        assert!(rem <= 90, "{rem} <= 90");
        assert!(rem >= 85, "{rem} >= 85");
        assert!(status.remaining_secs.is_none());
    }

    #[tokio::test]
    async fn get_quick_status_impl_disabled_when_both_kinds_off() {
        let s = Settings {
            micro_enabled: false,
            long_enabled: false,
            ..Settings::default()
        ...
        let (_dir, sched) = test_scheduler(s);
        let status = get_quick_status_impl(&sched).await;
        assert_eq!(status.state, QuickPanelState::Disabled);
        assert!(status.remaining_secs.is_none());
    }

    #[tokio::test]
    async fn get_quick_status_impl_on_break() {
        use crate::scheduler::types::BreakEvent;
        let (_dir, sched) = test_scheduler(Settings::default());
        *sched.current_break.lock().unwrap() = Some(BreakEvent {
        ...
        let status = get_quick_status_impl(&sched).await;
        assert_eq!(status.state, QuickPanelState::OnBreak);
    }

    #[tokio::test]
    async fn get_quick_status_impl_suppressed() {
        ...
        let status = get_quick_status_impl(&sched).await;
        assert_eq!(status.state, QuickPanelState::Suppressed);
    }

    #[tokio::test]
    async fn get_quick_status_impl_picks_sooner_break_not_tray_target() {
        let s = Settings {
        ...
        let status = get_quick_status_impl(&sched).await;
        assert_eq!(status.state, QuickPanelState::Running);
        let secs = status.remaining_secs.expect("running");
        assert!(
            secs <= 60,
            "panel should follow the sooner (micro) timer, got {secs}"
        );
    }
