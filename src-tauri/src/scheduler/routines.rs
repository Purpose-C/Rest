...
pub fn starter_routines() -> Vec<Routine> {
    ...
    vec![
        ...
        routine(
            ...
            Moderate,
            vec![
                step("慢慢握拳，然后五指充分张开。", 5),
                step("双向顺畅转动每侧手腕数次。", 6),
                step("轻轻向后扳手掌，拉伸前臂肌肉。", 6),
                step("轻松抖动双手，彻底放松。", 3),
            ],
        ),
        routine(
            ...
            Moderate,
            vec![
                step("站起身，双臂向上充分伸展。", 20),
                step("上身缓慢前倾下垂，自然放松头部。", 25),
                step("缓慢起身，然后向左右两侧转体。", 25),
                step("打开胸腔 —— 双手在背后交扣拉伸。", 20),
                step("脚跟着地，拉伸小腿肌肉。", 30),
                step("抖动四肢，放松全身。", 20),
            ],
        ),
        ...
        routine(
            ...
            Moderate,
            vec![
                step("坐姿猫牛式：缓慢交替拱起与伸展背部。", 30),
                step("坐姿脊柱扭转：先向右，再向左。", 40),
                step("单臂举过头顶侧弯拉伸，然后换边。", 40),
                step("上身向前俯在膝盖上，彻底放松颈部。", 30),
                step("挺直坐好，转动双肩完成。", 20),
            ],
        ),
        routine(
            ...
            vec![
                step("舒适坐好，可以闭上双眼。", 20),
                step("吸气 4 拍，呼气 6 拍 —— 保持节奏轻松。", 60),
                step("每次呼气时，感受肩膀自然下沉。", 60),
                step("在返回工作前，感知周围的空间。", 40),
            ],
        ),
    ]
}
...
mod tests {
    ...
    fn all_routines_appends_custom_and_drops_starter_id_collisions() {
        ...
        let eye = all
            ...
            .collect::<Vec<_>>();
        assert_eq!(eye.len(), 1);
        assert_eq!(eye[0].label, "眼部放松 (20-20-20)");
    }
